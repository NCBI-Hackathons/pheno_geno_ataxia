...
def user_input_from_gui(min_score, max_score='NO'):
    '''
    ...
    if max_score == 'NO':
        ...
        fname = str(round(min_score, 2)) + '.csv'
        data_.to_csv(fname, index=False)
    elif isinstance(max_score, int) or isinstance(max_score, float):
        data_ = data[(data['score_wo_prcnt'] >= min_score) & (data['score_wo_prcnt'] <= max_score)]
        fname = str(round(min_score, 2)) + '-' +str(round(max_score, 2)) + '.csv'
        data_.to_csv(fname, index=False)
user_input_from_gui(1, max_score='NO')
